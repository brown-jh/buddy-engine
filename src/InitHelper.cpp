...
void InitHelper::initWindow()
{
  ...
  window = glfwCreateWindow(WIDTH, HEIGHT, "Shmog", nullptr, nullptr);
  glfwSetWindowUserPointer(window, this);
  glfwSetFramebufferSizeCallback(window, framebufferResizeCallback);

}

void InitHelper::framebufferResizeCallback(GLFWwindow* window, int width, int height)
{
    auto app = reinterpret_cast<InitHelper*>(glfwGetWindowUserPointer(window));
    app->framebufferResized = true;
}

void InitHelper::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
    ...
    // BOZO create custom allocator that splits up a single allocation among many diff objects using the offset parameters
...
void InitHelper::createDescriptorPool()
{
...
void InitHelper::createDescriptorSets()
{
...
void InitHelper::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
    ...
    // BOZO: May want separate commannd pools for these buffers for optimization
...
void InitHelper::createIndexBuffer()
{
...
void InitHelper::createVertexBuffer()
{
    VkDeviceSize bufferSize = sizeof(vertices[0]) * vertices.size();
    // local device vertex buffer (we can write to this)
    VkBuffer stagingBuffer;
    VkDeviceMemory stagingBufferMemory;
...
uint32_t InitHelper::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void InitHelper::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    beginInfo.flags = 0;
...
void InitHelper::createSyncObjects()
{
    ...
    std::cout << "created sync objects\n";
}

void InitHelper::createFramebuffers()
{
...
void InitHelper::createCommandBuffers()
{
...
void InitHelper::createCommandPool()
{
...
void InitHelper::waitDeviceIdle()
{
...
void InitHelper::pickPhysicalDevice()
{
  ...
  if (device_ratings.rbegin()->first > 0)
  {
    ...
    {
        std::cout << "SELECTED GPU IS DISCRETE\n";
    }
    

    std::cout << "Suitable device found and selected\n";
  }
  else
  {
...
void InitHelper::createUniformBuffers()
{
...
void InitHelper::createDescriptorSetLayout()
{
...
void InitHelper::updateUniformBuffer(uint32_t currentImage)
{
...
void InitHelper::drawFrame()
{
    ...
    // reset command buffer so we can record
...
void InitHelper::recreateSwapChain()
{
    ...
    // BOZO: we may also need to recreate the renderpass
    // (in instances of moving from SDR to HDR)
...
void InitHelper::cleanupSwapChain()
{
